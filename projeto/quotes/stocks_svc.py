...
def populate_db_with_stock_company_names():
    ...
    api_request = requests.get(
        f"https://brapi.dev/api/quote/{names}?range=3mo&interval=1d"
    )
    ...
    for stock in all_stocks:
        ...
        instance.company_name = (
            stock["longName"] if "longName" in stock.keys() else None
        )
        instance.save()
...
def get_1mo_historical_prices():
    ...
    for name in names:
        api_request = requests.get(
            f"https://brapi.dev/api/quote/{name}?range=1mo&interval=1d"
        )
