...
class Crontab(object):
    ...
    def read(self):
        ...
        """
        self.crontab_lines = os.popen(
            "%s -l" % self.settings.CRONTAB_EXECUTABLE
        ).readlines()
                    ...
                    "command": " ".join(
                        filter(
                            ...
                            [
                                ...
                                "run",
                                self.__hash_job(job),
                                "--settings=%s" % self.settings.DJANGO_SETTINGS_MODULE
                                if self.settings.DJANGO_SETTINGS_MODULE
                                else "",
                                job_suffix,
                    ...
                    print(
                        ...
                        % (
                            ...
                            self.__get_job_by_hash(
                                job[0][2][job[0][2].find("crontab run") + 12 :].split()[
                                    0
                                ]
                            ),
                        )
    ...
    def run_job(self, job_hash):
        ...
        job_name = job[1]
        job_args = (
            job[2] if len(job) > 2 and not isinstance(job[2], string_type) else []
        )
